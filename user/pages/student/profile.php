...
$sql_student_info = "SELECT email, address, contact, firstname, middlename, lastname, grade_level, department, section, usertype FROM user_account WHERE user_id=?";
$stmt = $conn->prepare($sql_student_info);

if ($stmt) {
  $stmt->bind_param("i", $user_id);
  $stmt->execute();
  $stmt->bind_result($email, $address, $contact, $firstname, $middlename, $lastname, $grade_level, $department, $section, $usertype);
  $stmt->fetch();
  $stmt->close();
}
?>
