      ...
      <div class="main-panel">
        <div class="header-sticky">
          <div class="header-links">
            <a class="btn-success" href="class_course.php?class_id=<?php echo $class_id ?>&tc_id=<?php echo $tc_id ?>"><i
            ...
            <a href="todolist_missing.php?class_id=<?php echo $class_id ?>&tc_id=<?php echo $tc_id ?>&user_id=<?php echo $user_id ?>"
              class="nav-link active">Missing</a>
        ...
        <div class="content-wrapper align-items-center justify-content-center" style="margin-top: 10vh;">
            ...
            <div class="col-md-3">
              <div class="card">
                <div class="card-body">
                  ...
                  <span class="text-body-secondary ml-1">(Missing)</span>
                </div>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col">
              <?php
              $sql_update_status_assignment = "UPDATE classwork_assignment SET assignment_status = 'missing' WHERE 
                teacher_id = ? AND class_name = ? AND assignment_status = 'assigned' AND due_date < DATE(NOW())";
              $stmt_update_status_assignment = $db->prepare($sql_update_status_assignment);
              $stmt_update_status_assignment->execute([$teacher_id, $class_name]);

              $sql_update_status_question = "UPDATE classwork_question SET question_status = 'missing' WHERE 
                teacher_id = ? and class_name = ? AND question_status = 'assigned' AND due_date < DATE(NOW())";
              ...
              $sql_update_status_exam = "UPDATE classwork_exam SET exam_status = 'missing' WHERE 
                teacher_id = ? and class_id = ? AND exam_status = 'assigned' AND due_date < DATE(NOW())";
              $stmt_update_status_exam = $db->prepare($sql_update_status_exam);
              $stmt_update_status_exam->execute([$teacher_id, $tc_id]);

              $sql_update_status_quiz = "UPDATE classwork_quiz SET quiz_status = 'missing' WHERE 
                teacher_id = ? and class_id = ? AND quiz_status = 'assigned' AND due_date < DATE(NOW())";
              $stmt_update_status_quiz = $db->prepare($sql_update_status_quiz);
              $stmt_update_status_quiz->execute([$teacher_id, $tc_id]);
              ...
              $sql_quiz = "SELECT q.quiz_id, q.quizTitle AS title, q.due_date 
                FROM classwork_quiz q
                LEFT JOIN quizgrade sq
                ON q.quiz_id = sq.quiz_id AND sq.student_id = ?
                WHERE q.teacher_id = ? AND q.class_id = ? 
                AND q.quiz_status = 'missing'
                AND sq.quiz_id IS NULL";
              $stmt_quiz = $db->prepare($sql_quiz);
              $stmt_quiz->execute([$user_id, $teacher_id, $tc_id]);
              $quiz_results = $stmt_quiz->fetchAll();

              $sql_exam = "SELECT e.exam_id, e.examTitle AS title, e.due_date 
                FROM classwork_exam e
                LEFT JOIN examgrade se
                ON e.exam_id = se.exam_id AND se.student_id = ?
                WHERE e.teacher_id = ? AND e.class_id = ? 
                AND e.exam_status = 'missing'
                AND se.exam_id IS NULL";
              $stmt_exam = $db->prepare($sql_exam);
              $stmt_exam->execute([$user_id, $teacher_id, $tc_id]);
              ...
              foreach ($combined_results as $row) {
                if (isset($row['assignment_id'])) {
                  ...
                  ?>
                  <div class="d-grid gap-2 col-10 mx-auto mb-3 mt-4">
