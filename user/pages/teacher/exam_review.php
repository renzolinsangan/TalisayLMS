...
$sql_selectExam = "SELECT * FROM classwork_exam WHERE exam_id = ? AND class_id = ? AND teacher_id = ?";
$stmt_selectExam = $db->prepare($sql_selectExam);
$stmt_selectExam->execute([$exam_id, $class_id, $teacher_id]);
$result = $stmt_selectExam->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_POST['editGrade'])) {
  ...
  $sqlEditPoint = "UPDATE examgrade SET score = $score WHERE student_id = ? AND exam_id = ?";
  $stmtEditPoint = $db->prepare($sqlEditPoint);
  $stmtEditPoint->execute([$student_id, $exam_id]);
  header("Location: exam_review.php?class_id=$class_id&exam_id=$exam_id");
}
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="content-wrapper">
          <div class="row mb-4">
            <div class="col">
              ...
              <p class="text-body-secondary">(Exam Review)</p>
