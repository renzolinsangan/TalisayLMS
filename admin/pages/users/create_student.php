...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO user_account (username, password, email, firstname, middlename, lastname, department, usertype, address, contact, grade_level, section) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmtinsert = $db->prepare($sql);
    $result = $stmtinsert->execute([$username, $hashedPassword, $email, $firstname, $middlename, $lastname, $department, $usertype, $address, $contact, $grade_level, $section]);

    if ($result) {
        header("Location: student.php?msg=Student created successfully!");
    } else {
    ...
    <script>
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (usernameInput.value === '' || passwordInput.value === '' || emailInput.value === '' ||
                ...
                gradelevelDropdown.value === '' || sectionInput.value === '' ||
